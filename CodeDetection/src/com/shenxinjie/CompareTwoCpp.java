...
public class CompareTwoCpp {
	...
	private static int[][] MatchResult;//两个文件的ii行和jj行相同则为1， 不同则为0.此项用于单击某一行，显示对应标红
	private static int[] FileLength;

	
	public static Results getResult(ArrayList<String> filePathList, int filenum1, int filenum2)
	{
		...
		MatchResult = new int [MAXLINE][MAXLINE];
		MatchR = new int [2][MAXLINE];
		IndexOfLine = new int[2][MAXLINE];
		
		FileData = new String[2][MAXLINE];
		ChuLi = new String[2][MAXLINE];
		Sim = new double[2][2];
		FileDataModified = new String[MAXSTU][MAXLINE];
		FileLength = new int[2];
		
		
		tree = new FileTree[2];//声明2棵树
		tree[0] = new FileTree(0, "#Root");
		tree[1] = new FileTree(1, "#Root");
		ReadData(FileNameList[filenum1], 0);/**
										 * 读取第i个文件每一行的字符存储在FileData中,不包含空行。
										 * 去除头文件、注释后存储在ChuLi中。
										 * 分割单词，每行的单词存储在line中，每一行的line存储在word中，每一个文件的word存储在Words中。
										 * 对分割完单词后的Words添加花括号。
										 */										
		Tongji(tree[0], 0);//建立语法树
		ReadData(FileNameList[filenum2], 1);
		Tongji(tree[1], 1);
		
		double sim1 = cal_similarity(tree[0].root, tree[1].root);
...
//		Words = null;因为Words定义在此函数外面，所以不能设置null，第二次会有问题。
	...
	private static void PrintInf(double sim1)
	{
		// TODO Auto-generated method stub	
		String[] simResults = new String[1];
		Double sim_d = new Double(sim1);
		simResults[0] = sim_d.toString();
		results = new Results(FileData, simResults, 1, MatchResult, MatchR, FileLength);
	}
...
//		System.out.println("keyword:"+tree1.key_word + " "+tree2.key_word);
//		System.out.println(tree1.hashs);
//		System.out.println(tree2.hashs);
//		System.out.println(tree1.pos);
//		System.out.println(tree2.pos);
		...
		while (dp[i][j] != 0) {
			if (dp[i][j] == dp[i-1][j]) {
				...
				i--;
				j--;
				int i_th = tree1.pos.get(i);
				int j_th = tree2.pos.get(j);
				MatchResult[i_th][j_th] = 1;
				MatchR[0][i_th] = 1;
				MatchR[1][j_th] = 1;
//				System.out.println("MatchResult: "+i_th+" "+j_th);
			}
		}
						...
						if (st.size() != 0) {
							...
							if (st.size() != 1) {
								...
								for (int th1 = 0; th1 < len; th1++) {
									...
									st.peek().pos.add(old_top.pos.get(th1));
								}
							}
			...
			if (str_line.length() != 0) {
				if (st.size() == 0) {
					...
					if (str_line.length() != 0) {
						...
						st.peek().pos.add(IndexOfLine[th][i]);
//						System.out.println("00000: "+str_line);
					}
				}
		  ...
		  try {
				...
				if(file.isFile() && file.exists()){ //判断文件是否存在
				  ...
				  while((lineTxt = bufferedReader.readLine()) != null){
					...
					if(FileData[i][x-1].length()==0) --x;//排除空行
					else {
						/**
						...
						if (ChuLi[i][y].length() != 0) {
							...
							if (havewords == 1) {
								IndexOfLine[i][y] = x-1;//记录在原始记录上对应的行号。
								y++;//非空再变为下一行
								havewords = 0;
							}
						}
	...
	private static void ProcessFileName(ArrayList<String> fileList)
	{
		...
		{
			System.out.println("CompareTwoCpp: " + (String)fileList.get(i));
			FileNameList[i] = (String)fileList.get(i);
		}
       	
		System.out.println("CompareTwoCpp: FileNumber: " + FileNumber);
	}
